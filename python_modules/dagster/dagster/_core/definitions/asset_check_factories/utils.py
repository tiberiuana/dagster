...
def retrieve_latest_parseable_observation_or_materialization(
    ...
    even though there might be a more recent observation that we can parse.

    The reason we do this is to avoid an expensive query into the long-tail history of the asset.
    Retrieving the latest record of a particular type is cheap. Retrieving the latest N records is not.
